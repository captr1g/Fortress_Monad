...
contract MorphoLeverageExecutorFuzzTest is Test {
    ...
    function testFuzz_slippageEnforced(
        ...
        bytes memory swapCalldata = abi.encodeCall(
            MockDex.swapExact,
            (address(usdc), swapIn, address(collat), collatOut, address(lev))
        );
    ...
    function testFuzz_debtAlwaysEqualsFlash(
        uint256 inputAssets,
        uint256 flashAssets,
        uint256 collatOut
    ) public {
    ...
    function testFuzz_residualReturnedToUser(
        uint256 inputAssets,
        uint256 flashAssets,
        uint256 consumed
    ) public {
        ...
        bytes memory swapCalldata = abi.encodeCall(
            MockDex.swapExact,
            (address(usdc), consumed, address(collat), collatOut, address(lev))
        );
        MorphoLeverageExecutor.LeverageParams memory p = MorphoLeverageExecutor
            .LeverageParams({
                market: _market(),
    ...
    function testFuzz_rateSwapConsistent(
        uint256 inputAssets,
        uint256 flashAssets,
        uint256 rate
    ) public {
        ...
        bytes memory swapCalldata = abi.encodeCall(
            MockRateDex.swapAtRate,
            (address(usdc), swapIn, address(collat), rate, address(lev))
        );
        MorphoLeverageExecutor.LeverageParams memory p = MorphoLeverageExecutor
            .LeverageParams({
                market: _market(),
