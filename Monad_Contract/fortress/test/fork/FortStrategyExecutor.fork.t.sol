...
contract FortStrategyExecutorForkTest is Test {
    ...
    function setUp() public {
        ...
        ERC1967Proxy morphoProxy = new ERC1967Proxy(
            address(morphoImpl),
            abi.encodeCall(MorphoStrategyAdapter.initialize, (address(executor), address(this)))
        );
    ...
    function _swapStepData(
        ...
        bytes memory swapCalldata = abi.encodeCall(
            MockSwapTarget.swap,
            (USDC, amountIn, yoUSD, amountOut, address(swapAdapter))
        );
        return
            abi.encode(
                address(mockSwap),
    ...
    function _swapBalanceData(
        address tokenIn,
        uint256 rateWad,
        uint256 minAmountOut
    ) internal view returns (bytes memory) {
        bytes memory swapCalldata = abi.encodeCall(
            MockSwapTarget.swapBalance,
            (tokenIn, yoUSD, rateWad, address(swapAdapter))
        );
        return
            abi.encode(
                address(mockSwap),
    ...
    function test_fork_swapAndSupply() public {
        ...
        (, , uint128 collateral) = IMorphoBlueExtended(MORPHO_BLUE).position(
            MARKET_ID,
            user
        );
    ...
    function test_fork_oneLoop() public {
        ...
        IFortStrategyExecutor.Step[]
            memory steps = new IFortStrategyExecutor.Step[](3);
        steps[0] = IFortStrategyExecutor.Step({
            adapterId: SWAP_ID,
            action: IStrategyAdapter.ActionType.SWAP,
            tokenIn: USDC,
    ...
    function test_fork_leverageLoop() public {
        ...
        IFortStrategyExecutor.Step[]
            memory steps = new IFortStrategyExecutor.Step[](17);
        ...
        assertGt(
            collateral,
            90e6,
            "leveraged collateral should exceed initial"
        );
        assertGt(borrowShares, 0, "borrowShares should be positive");
    }
