...
export async function getCachedRates(redis: Redis, marketId: string): Promise<StoredRates | null> {
  const raw = await redis.get(`${KEY_PREFIX}${marketId}`);
  if (!raw) return null;
  return JSON.parse(raw) as StoredRates;
}

export async function setCachedRates(redis: Redis, marketId: string, rates: StoredRates): Promise<void> {
  await redis.set(`${KEY_PREFIX}${marketId}`, JSON.stringify(rates), "EX", CACHE_TTL_SECONDS);
}

export async function deleteCachedRates(redis: Redis, marketId: string): Promise<void> {
