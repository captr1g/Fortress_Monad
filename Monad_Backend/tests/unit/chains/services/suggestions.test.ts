...
describe("suggestionsForError — typed errors", () => {
  ...
  it("returns no suggestions for an unsupported amount override", () => {
    expect(suggestionsForError(new UnsupportedAmountOverride("leverage"), MONAD_CHAIN_ID)).toEqual([]);
  });
});

describe("suggestionsForError — planner refusals", () => {
  it("points at the supported token list on an unknown-token refusal", () => {
    const out = suggestionsForError(new PlannerRefusal("Unknown token FOO not in the list"), MONAD_CHAIN_ID);
  ...
  it("gives a concrete starting line on a missing-amount refusal", () => {
    const out = suggestionsForError(new PlannerRefusal("Could not parse an amount"), MONAD_CHAIN_ID);
    expect(out).toHaveLength(1);
    expect(out[0].insertText).toMatch(/I have 1 USDC on Monad\./);
  });

  it("returns [] for a generic refusal with no actionable hint", () => {
    expect(suggestionsForError(new PlannerRefusal("request is out of scope"), MONAD_CHAIN_ID)).toEqual([]);
  });
});
...
describe("suggestionsForError — builder/resolver failures", () => {
  ...
  it("returns [] for an unrelated error", () => {
    expect(suggestionsForError(new Error("network timeout"), MONAD_CHAIN_ID)).toEqual([]);
  });
  ...
  it("handles non-Error thrown values without crashing", () => {
    expect(suggestionsForError("a plain string", MONAD_CHAIN_ID)).toEqual([]);
  });
});
