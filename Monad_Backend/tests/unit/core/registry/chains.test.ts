...
import { TOKENS, MONAD_CHAIN_ID } from "../../../datasets/monad.js";

beforeAll(() => seedRegistry());

describe("chain registry lookups", () => {
  it("finds Base by key and by id, and getChain aliases getChainById", () => {
    const byKey = getChainByKey("monad");
    const byId = getChainById(MONAD_CHAIN_ID);
    expect(byKey?.chainId).toBe(MONAD_CHAIN_ID);
    expect(byId?.chainKey).toBe("monad");
    expect(getChain(MONAD_CHAIN_ID)).toBe(byId);
  });
...
describe("token lookups", () => {
  it("finds a token by symbol (case-insensitive)", () => {
    expect(findToken("monad", "usdc")?.address).toBe(TOKENS.USDC);
    expect(findToken("monad", "USDC")?.address).toBe(TOKENS.USDC);
  });

  it("finds a token by address (case-insensitive)", () => {
    expect(findToken("monad", TOKENS.WETH.toLowerCase())?.symbol).toBe("WETH");
  });

  it("returns undefined for unknown token or unknown chain", () => {
    expect(findToken("monad", "NOTATOKEN")).toBeUndefined();
    expect(findToken("nochain", "USDC")).toBeUndefined();
  });

  it("findTokenById mirrors findToken via chainId", () => {
    expect(findTokenById(MONAD_CHAIN_ID, "WETH")?.address).toBe(TOKENS.WETH);
  });

  it("tokenAddress returns the checksummed address or undefined", () => {
    expect(tokenAddress(MONAD_CHAIN_ID, "USDC")).toBe(TOKENS.USDC);
    expect(tokenAddress(MONAD_CHAIN_ID, "NOPE")).toBeUndefined();
  });
});
