/**
 * Does a vault-adapter DEPOSIT into Pendle 40acresUSDC simulate cleanly?
 * If deposit works via the vault adapter but redeem doesn't, the adapter has a
 * direction-specific limitation (not a whole-market incompatibility).
 *
 * Run: node --import tsx scripts/debug-pendle-deposit.ts
 */
...
async function main() {
  ...
  const { transactions, description } = await builder.build(
    {
      ...
      amount: AMOUNT,
      allocations: [{ protocol: "Pendle", bps: 10000, pendleMarket: "40acresUSDC" }],
  ...
  transactions.forEach((tx, i) => {
    try {
      ...
      if (d.functionName === "approve") console.log(`  tx${i + 1} approve`, (d.args as any[])[0], (d.args as any[])[1].toString());
    } catch { console.log(`  tx${i + 1} → ${tx.to} (vault deposit)`); }
  });
